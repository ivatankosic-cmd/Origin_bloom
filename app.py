...
st.markdown("""
    ...
    /* Crno dugme sa zlatnim efektom za formu */
...
if st.session_state.prikazano:
    ...
    if not location:
        ...
        try:
            ...
            st.markdown(f"""<div class='result-section'><h2 class='result-title'>Tvoj Origin Bloom profil je kreiran.</h2><p class='result-subtitle'>Precizni podaci prevedeni su u tvoj jedinstveni botanički i koloritni potpis.</p><div class='astrology-header'>Znak: <b>{znak_ime}</b> &nbsp;|&nbsp; Podznak: <b>{podznak_ime}</b></div>""", unsafe_allow_html=True)
            ...
            # SEKCIJA ZA NARUČIVANJE SA NOVIM TEKSTOM
            ...
            with st.form("forma_za_narucivanje", clear_on_submit=False):
                ...
                if submit_narudzbina:
                    if ime_prezime.strip() and email.strip() and telefon.strip() and adresa.strip() and saglasnost_vizija and saglasnost_vreme:
                        ...
                        poruka = f"NOVA PORUDŽBINA - Origin Bloom\n\nIme: {ime_prezime}\nEmail: {email}\nTelefon: {telefon}\nAdresa: {adresa}\nNamena: {namena}\nZnak: {znak_ime} | Podznak: {podznak_ime}"
                        msg = MIMEText(poruka)
                        msg['Subject'] = f'Nova porudžbina: {ime_prezime} (Origin Bloom)'
                        
                        try:
                            # Povlačenje podataka iz Streamlit Secrets
